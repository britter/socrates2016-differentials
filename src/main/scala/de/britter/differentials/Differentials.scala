...
object Differentials {
  ...
  def solveDifferentialEquation(startwert: Float, ableitung: Differential): Loesung = {
  ...
  def main(args: Array[String]): Unit = {
    val funktionsApproximation: Loesung = solveDifferentialEquation(1, v => v * 1.1f)
    ...
    println(s"actual: $zweiterFunktionswert expected: ${Math.exp(1.1 * zweiterFunktionswert._1)}")
    println(s"actual: $thirdFunktionswert expected: ${Math.exp(1.1 * thirdFunktionswert._1)}")
